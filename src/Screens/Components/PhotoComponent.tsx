...
const PhotoComponent: React.FC<Props> = ({photo,navigation}) => {
    ...
    return(
        <TouchableOpacity style={styles.rootView} onPress={() => navigation.navigate("DescriptionScreen",{
            photoId: photo.id
        })}>
            <Image source={{uri : getImageUrl(photo.server,photo.id,photo.secret,true)}} style={styles.imageStyle} resizeMode="cover" />
